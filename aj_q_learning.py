...
def simulate():
    ...
    for episode in range(NUM_EPISODES):
        ...
        #for each timestep in the max number of timesteps
        for t in range(MAX_T):
            ...
            # Observe the result, place into bucket
...
def get_explore_rate(t):
    holder = float(float(t+1)/float(25))
    return max(MIN_EXPLORE_RATE, min(1, 1.0 - math.log10(holder)))

def get_learning_rate(t):
    holder = float(float(t+1)/float(25))
    return max(MIN_LEARNING_RATE, min(0.5, 1.0 - math.log10(holder)))
